...
(function (global, factory) {
  ...
  function initRender (vm) {
    ...
    var options = vm.$options;
    var parentVnode = vm.$vnode = options._parentVnode; // the placeholder node in parent tree（父节点树中的占位符节点）
    // 插槽相关
  ...
  function initEvents (vm) {
    ...
    // init parent attached events（初始化父节点相关事件）
  ...
  function setActiveInstance(vm) {
    var prevActiveInstance = activeInstance; // 缓存旧的实例
  ...
  function lifecycleMixin (Vue) {
    ...
    Vue.prototype._update = function (vnode, hydrating) {
      var vm = this;
      // 数据更新时会用到这些变量
      var prevEl = vm.$el; // 旧的挂载元素
      var prevVnode = vm._vnode; // 旧的渲染VNode
      var restoreActiveInstance = setActiveInstance(vm); // 设置为全局当前激活实例
  ...
  function initGlobalAPI (Vue) {
    ...
    // components with in Weex's multi-instance scenarios.（缓存Vue大类）
  ...
  function createPatchFunction (backend) {
    ...
    /**
     * 把真实DOM元素插入到父节点中
     * @params parent: 父节点（真实DOM）
     * @params elm: 要插入的节点（真实DOM）
     * @params ref: 在哪个元素前插入
     */
    ...
    /**
     * 检查key => 同一层级是否定义了重复的key（避免更新错误）
     * @params children：Vnode的子节点
     */
    ...
    return function patch (oldVnode, vnode, hydrating, removeOnly) {
      ...
      if (isUndef(oldVnode)) {
        // oldVnode不存在（可能是一个组件），创建一个新的根元素
        // empty mount (likely as component), create new root element
        isInitialPatch = true;
        createElm(vnode, insertedVnodeQueue);
      } else {
        var isRealElement = isDef(oldVnode.nodeType); // 是否为真实DOM元素
        if (!isRealElement && sameVnode(oldVnode, vnode)) {
          // 不是真实DOM元素
          // patch existing root node（对比更新）
          ...
          var oldElm = oldVnode.elm; // 旧的挂载元素
          var parentElm = nodeOps.parentNode(oldElm); // 挂载元素的父节点
